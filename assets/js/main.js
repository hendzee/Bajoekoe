   ...
   if (this.checked) {
	...
	function adjustIsotopePrices(min, max) {
		...
		$('#isotopeContainer .isotope-item').removeClass('priced').each(function () {
			var $e = $(this);
			var price = $('span.price', $e).text().replace('Rp.', '');
	...
	(function ($) {
		if (jQuery().slider) {
			...
			$('body').on('filters.reset', function () {
				...
				$('.min', $filterValues).val('Rp.' + fMin);
				$('.max', $filterValues).val('Rp.' + fMax);
			...
			$filterWidget.slider({
				...
				values: [fMin, fMax],
				slide: function (e, ui) {
					$('.min', $filterValues).val('Rp.' + ui.values[0]);
					$('.max', $filterValues).val('Rp.' + ui.values[1]);
				},
				stop: function (e, ui) {
					adjustIsotopePrices(ui.values[0], ui.values[1]);
				}
			});
			$('.min', $filterValues).val('Rp.' + $filterWidget.slider("values", 0));
			$('.max', $filterValues).val('Rp.' + $filterWidget.slider("values", 1));
			$('.min, .max', $filterValues).on('input',function () {
				...
				if ($(this).hasClass('max')) {
					$filterWidget.slider("values", 1, $(this).val().replace(/\D/g, ''));
								...
								if (!($(this).val().charAt(0) == "Rp.")) {
									$(this).val('Rp.' + $(this).val());
								}
							}).on('focus', function () {
								if ($(this).val().charAt(0) == "Rp.") {
