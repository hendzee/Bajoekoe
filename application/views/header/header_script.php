<!DOCTYPE html>
<!--open html to body-->
<html>
<head>
  <meta charset="utf-8">
  <meta name="description" content=" ">
  <meta name="author" content=" ">
  <meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no, maximum-scale=1, shrink-to-fit=no">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <script src="<?=base_url()?>assets/cdn-cgi/apps/head/VLyxQxfym5OKy1INipgd4pei8TM.js"></script><link rel="shortcut icon" href="<?=base_url()?>assets/favicon.ico" type="image/x-icon">

  <title>Bajoekoe</title>

  <!---product_list-->
  <link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/chosen.css">
  <link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/jquery-ui-1.10.3.custom.min.css">
  <link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/prettyPhoto.css">

  <link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/flexslider.css">
  <link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/font-awesome.css">
  <link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/bootstrap.css">
  <link rel="stylesheet" type="text/css" href="<?=base_url()?>assets/css/style.css">  
</head>